...
function loadStep(name) {
  ...
  // R-side dashed bonds: R->TS broken (red), R->TS formed (green)
  const rDashes = [];
  d.rt_changes.forEach(([i, j, wR, wT]) => {
    const color = wT < wR ? 'red' : 'green';
    rDashes.push([i, j, color, color === 'red' ? 'R→TS↓' : 'R→TS↑']);
  });

  // P-side dashed bonds: TS->P broken (red), TS->P formed (green)
  const pDashes = [];
  d.tp_changes.forEach(([i, j, wT, wP]) => {
    const color = wP < wT ? 'red' : 'green';
    pDashes.push([i, j, color, color === 'red' ? 'TS→P↓' : 'TS→P↑']);
  });

  // TS-side dashed bonds: in-flight bonds (orange) + R->P changes still active
  const tDashes = [];
  d.inflight.forEach(([i, j, wR, wT, wP]) => {
    tDashes.push([i, j, '#ffa500', `WBO=${wT.toFixed(2)}`]);
  });
  // Also overlay R->TS and TS->P breaks/forms on TS panel
  d.rt_changes.forEach(([i, j, wR, wT]) => {
    const c = wT < wR ? 'red' : 'green';
    tDashes.push([i, j, c, c === 'red' ? 'R→TS↓' : 'R→TS↑']);
  });
