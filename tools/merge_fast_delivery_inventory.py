...
def main():
    ...
    definitions.extend(
        (Path(path).stem, Path(path), "Building Block ID", "supplement_ids")
        for path in args.supplement)
    records = {}
    counts = Counter()
    for source, path, id_column, id_key in definitions:
    ...
    fields = (
        ...
        "Inventory IDs", "Supplement IDs", "InChIKey", "Merged Row Count",
    )
    with gzip.open(output, "wt", encoding="utf-8", newline="") as stream:
        writer = csv.DictWriter(stream, fieldnames=fields)
        writer.writeheader()
        for key, record in records.items():
            identifiers = (record["inventory_ids"]
                           or record["supplement_ids"]
                           or record["mcule_ids"])
            writer.writerow({
                ...
                "Supplement IDs": ";".join(record["supplement_ids"]),
    ...
    input_rows = {
        ...
        for source, _path, _id_column, _id_key in definitions
    }
    summary = {
        "schema": "rxn_core.merged_fast_delivery_with_inventory/v2",
        "inputs": {
            source: str(path)
            for source, path, _id_column, _id_key in definitions
        },
