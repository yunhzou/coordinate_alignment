...
function scoreCards(a){{const s=a.score;const cell=(label,value,detail,cls='')=>'<div class="scorecell '+cls+'"><small>'+label+'</small><b>'+value+'</b><small>'+detail+'</small></div>';return '<div class="scoregrid">'+cell('Pareto rank',a.ground_truth?'—':(a.pareto_layer||'—'),a.ground_truth?'validation only':'same rank = trade-offs')+cell('Bond breaking',s.broken_bonds===undefined?'—':s.broken_bonds,'source boundary cuts','breaking')+cell('Bond forming',s.formed_bonds===undefined?'—':s.formed_bonds,'target connections','forming')+cell('Total changes',s.broken_bonds===undefined||s.formed_bonds===undefined?'—':s.broken_bonds+s.formed_bonds,'breaking + forming')+cell('P coverage',(100*s.covered_target_atoms/s.target_atom_count).toFixed(1)+'%',s.covered_target_atoms+' / '+s.target_atom_count+' atoms incl. H')+cell('R retention',s.set_atom_retention===undefined?'—':(100*s.set_atom_retention).toFixed(1)+'%','unique P / all input atoms')+'</div>'}}
