...
main{{display:grid;grid-template-rows:auto minmax(220px,1fr) minmax(260px,1fr);min-width:0;overflow:auto}} #reactants{{display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:8px;padding:8px 8px 4px}}
...
#productWrap{{padding:4px 8px 8px;display:grid;grid-template-rows:auto minmax(0,1fr);min-height:0}} #Ppanel{{height:100%;min-height:160px}} .view{{position:absolute;inset:0}}
...
<main><section id="scorePlotPanel"><b>Click a score point to open its assembly</b><div class="plotnote">Blue: blind proposals · green: validation · ×N: coincident alternatives (no jitter). Upper left is better; trade-offs share a Pareto layer.</div><div class="controls"><label><input id="fragments" type="checkbox" checked> color fragments</label><label><input id="symmetry" type="checkbox"> show symmetry domains</label><label><input id="labels" type="checkbox"> sampled P# identities</label></div><svg id="scorePlot" aria-label="Retention versus structural changes"></svg><div id="scorePlotChoices"></div></section><div id="reactants"></div>
<div id="productWrap"><div id="selectedScores"></div><section class="panel" id="Ppanel"><div class="label" id="LP"></div><div class="view" id="P"></div></section></div></main></div>
