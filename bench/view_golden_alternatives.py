...
def build(source,audit,index,output,*,direction=None,pattern_seconds=0,path_seconds=1):
    ...
    extraction=None
    if pattern_seconds>0:
        extraction,patterns,equivalence=collect(directory/'cuts/aam.pkl.gz',aam,plan,(reactant,product),
    ...
    if extraction is not None:
        payload['pattern_extraction']=dict(complete=extraction['complete'],
            visited_paths=len(extraction['paths']),checkpoint=str(out/'pattern_checkpoint.json'))
...
<details><summary>All class counts and scores — click a row</summary><table><tr><th>Rank</th><th>Reference</th><th>Heavy P</th><th>Break</th><th>Form</th><th>Order</th><th>Terminals</th><th>Raw heavy</th><th>Explicit witnesses</th></tr>{buttons}</table></details></div>'''
    ...
    if extraction is not None:
        ...
        page=page.replace('<h2>Alternative atom assignments</h2>',
            ...
            f'{len(extraction["paths"])} paths visited. Original compressed archive retained; extraction can resume. '
...
if __name__=='__main__':
    ...
    summaries=[build(args.source,audit,index,args.output,direction=args.direction,
        pattern_seconds=args.pattern_seconds,path_seconds=args.path_seconds) for index in args.indices]
