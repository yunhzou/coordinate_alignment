...
let vR, vP;
function drawBondCylinders(viewer, atoms, pairs, color, labelText) {
  for (const [i, j] of pairs) {
    ...
    if (labelText) {
      ...
      viewer.addLabel(`${labelText} ${i}-${j}`, {
        position: mid,
        fontColor: 'white', backgroundColor: color, backgroundOpacity: 0.85,
...
function render(name) {
  ...
  drawBondCylinders(vR, atomsR, d.broken_R, 'red', 'BROKEN');
  ...
  drawBondCylinders(vP, atomsP, d.formed_P, 'green', 'FORMED');
