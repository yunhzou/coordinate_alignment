...
def test_recorded_candidate_roles_match(recorded):
    for args, kwargs, expected in recorded["roles"].calls:
        _assert_same(_fast.candidate_roles(*args, **kwargs), expected,
                     "candidate_roles")
...
def test_python_path_is_default_and_fast_path_is_opt_in():
    ...
    # with the variable set every public name is rebound to the extension,
    # and extend's imported names follow the selected implementation
    assert _probe("1") == [0, 1, 0, 1, 0, 0, 0, 0, 0]
